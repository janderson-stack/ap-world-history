...
function renderFirst10(){
  const rawTitle=first10.title||'';
  const readingTitle=rawTitle.replace(/^First\s*(?:&amp;|&)\s*10:\s*/i,'');
  const deck=T.subtitle||'';
  const primarySkill=(T.skill&&T.skill.title||'').replace(/^AP Skill Builder:\s*/i,'').trim();
  const skillTags=[primarySkill,'Argumentation','Historical Reasoning'].filter(Boolean);
  const sections=first10.paragraphs.map((p,i)=>`<div class="f10-section"><span class="f10-section-num">${i+1}</span><p class="f10-reading-text">${p}</p></div>`).join('');
  return `<div class="f10-title-band"><p class="f10-eyebrow">First &amp; 10 Reading</p><h2 class="f10-title">${readingTitle}</h2><p class="f10-deck">${deck}</p><div class="f10-skill-tags">${skillTags.map(s=>`<span class="f10-skill-tag">${s}</span>`).join('')}</div></div><div class="f10-reading-body">${sections}</div>${draft(`${T.id}-first10`,first10.prompt)}`;
}
